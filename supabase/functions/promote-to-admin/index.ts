...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Vérification si l'utilisateur a déjà le rôle admin
    ...
    if (existingRole && existingRole.length > 0) {
      return new Response(
        JSON.stringify({ message: "L'utilisateur est déjà un administrateur" }),
        { 
          status: 200, 
          headers: { ...corsHeaders, "Content-Type": "application/json" } 
        }
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `L'utilisateur ${email} est maintenant un administrateur`,
        userId: userId
      }),
      { 
        status: 200, 
